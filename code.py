...
target = input("[*] Target site? ").replace("http://","").replace("FrontPage","").replace("WikiSandBox","")
...
jar=None
print(("http://%s/WikiSandBox" % target))
...
if "Edit (Text)" in permission_check:
    ...
    check = False
if not check:
    have_acc = input("[*] Do you have an account? [Y/N] ").lower()
